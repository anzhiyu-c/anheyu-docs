...
<Tabs items={["新手用户", "生产环境", "开发者", "性能优先"]}>
  ...
  <Tab value="生产环境">
    ...
    **Docker Compose 优势：**
    - ✅ 服务编排和依赖管理 
    - ✅ 内置完整多媒体处理能力 (VIPS、FFMpeg、LibreOffice、FFProbe)
    - ✅ 容器化隔离，环境一致性
    - ✅ 便于扩展和负载均衡 
    - ✅ 无需手动配置复杂依赖
...
- **优势**:
  - 一键部署包含所有依赖服务
  - 内置完整多媒体处理组件 (VIPS、FFMpeg、LibreOffice、FFProbe)
  - 版本管理和升级简便
  - 环境隔离性好
  - 官方维护配置
  - 无需手动安装复杂依赖
...
- **优势**:
  ...
  - 资源占用最少
  - 跨平台支持
- **劣势**: 需要手动安装和配置多媒体处理组件 (VIPS、FFMpeg、LibreOffice 等)
  ...
  <Tab value="Docker Compose">
    ...
    <Callout type="info" title="Docker Compose 包含的完整服务">
      ...
      **多媒体处理组件：**
      ...
      - 📄 **LibreOffice** - 文档处理组件，支持 Office 文档的预览和转换
      - 🔍 **FFProbe** - 视频信息提取工具，获取视频文件的元数据信息
    ...
    <Callout type="info" title="多媒体处理组件说明">
      ...
      - **📄 LibreOffice** - Office文档处理，支持文档预览、PDF转换
      - **🔍 FFProbe** - 自动提取视频文件的元数据信息（分辨率、时长、编码等）
