...
<Tabs items={['Docker Compose', 'Docker', '源码部署', '二进制文件']}>
  <Tab value="Docker Compose">
        ...
        services:
          anheyu:
            ...
            environment:
              ...
              # 默认使用内置 Next.js 前端，无需配置
              # 独立模式：指向单独运行的前端容器
              # - ANHEYU_FRONTEND_URL=http://anheyu-frontend:3000
            volumes:
              - ./data:/anheyu/data
              - ./themes:/anheyu/themes
              - ./static:/anheyu/static
    ...
    <Callout type="info" title="v1.8.0+ Docker 架构说明">
      ...
      **从旧版本升级的用户**：只需拉取最新镜像并重启即可，`docker-compose.yml` 无需修改。
    </Callout>
      ...
      <Tab value="Pro 版">
        ...
        **方式 1：使用 Redis（推荐生产环境）**
        ...
        docker run -d \
          ...
          -v anheyu_pgdata:/var/lib/postgresql/data \
          -p 5432:5432 \
        ...
        **方式 2：不使用 Redis（自动降级到内存缓存）**
        ...
        <Callout type="warn" title="Pro 版注意事项">
          ...
          - PostgreSQL 是必需的依赖服务，推荐使用 `pgvector/pgvector:pg17` 镜像以支持向量搜索功能
  ...
  <Tab value="二进制文件">
    ...
    **前置依赖：**
    - **Node.js 20+**（必须）- 前端使用 Next.js SSR，需要 Node.js 运行时

    <Callout type="warn" title="Node.js 运行时要求">
      从 v1.8.0 开始，前端架构升级为 **Next.js**，采用服务端渲染（SSR）以获得更好的 SEO 和首屏性能。
      二进制启动后会自动通过 `node` 命令启动内置的 Next.js 前端服务，因此**系统必须安装 Node.js 20+**。
